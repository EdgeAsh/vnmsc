...
<div>
	...
	<nav-bread><span> OrderConfirm </span></nav-bread>
        ...
        <div class="price-count">
          <ul>
            <li>
              ...
              <span>{{subtotal | moneyFormat('￥')}}</span>
            </li>
            <li>
              <span>Shipping:</span>
              <span>{{shipping | moneyFormat('￥')}}</span>
            </li>
            <li>
              <span>Discount:</span>
              <span>{{discount | moneyFormat('￥')}}</span>
            </li>
            <li>
              <span>Tax:</span>
              <span>{{tax | moneyFormat('￥')}}</span>
            </li>
            <li class="order-total-price">
              <span>Order total:</span>
              <span>{{orderTotal | moneyFormat('￥')}}</span>
            </li>
          </ul>
        </div>
      </div>
	...
	methods:{
		init(){
			axios.get('/users/cartList').then((response)=>{
				let res = response.data,cartList = [];
				if(res.status=='0'){
					cartList = res.result;
